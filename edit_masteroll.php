<?php
	...
	if(isset($_POST['update'])){
		...
		$date = $_POST['date'];
		$no_of_labours = $_POST['no_of_labours'];
		$rate = $_POST['rate'];
		$amount = $_POST['amount'];
		...
		$id=$row['id'];
		$query = "UPDATE `masteroll` SET date='$date',no_of_labours='$no_of_labours',rate='$rate',amount='$amount',remarks='$remarks' where  user_id='$id' and activity_id='$activity_id'";
		if (mysqli_query($db,$query))

	{
		echo "<script>alert('successfully updated.')
				location.href ='edit_masteroll.php?id='$activity_id';
				</script>";
	}
	else
	{
...
<!-- Side bar end -->
	...
	<form method="post" action ="" style="position:fixed;margin-top:5px;margin-left:5px;border-radius:5px;">
	...
	include('connection.php');
	$a_id=$_GET['editid'];
	$sql = "SELECT masteroll.*,activity.* FROM masteroll,activity WHERE activity.id=masteroll.activity_id AND masteroll.activity_id='$a_id'";
	$results = mysqli_query($db,$sql) or die("database error:".mysqli_error($db));
	while($row = mysqli_fetch_assoc($results)){?>
	<ul>
	<li><b>Date:</b><input type="date" name="date" value="<?php echo $row['date'];?>" class="form-control1"  required></li>
	<li><b>No of Labours:</b><input type="text" name="no_of_labours" value="<?php echo $row['no_of_labours'];?>" class="form-control1" required></li>
	<li><b>Rate:</b><input type="text" name="rate" value="<?php echo $row['rate'];?>" class="form-control1"></li>
	</br></br>
	<li><b>Amount:</b><input type="text" name="amount" value="<?php echo $row['amount'];?>" class="form-control1"></li>
	<li><b>Remarks:</b>&nbsp;&nbsp;&nbsp;<input type="text"name="remarks" value="<?php echo $row['remarks'];?>" class="form-control1"></li></br></br>
