...
FROM activity A,contract_details C,users U,running_bill R
WHERE A.id = C.activity_id AND R.contract_details_id=C.id AND U.id = A.user_id GROUP BY R.contract_details_id ORDER BY C.id DESC LIMIT " .$this_page_first_result . ',' .$result_per_page;
...
<!-- Side bar start -->
	...
	<a href="chiefpage.php"><i class="fas fa-table"></i><span>Show Activity</span></a>
...
<!-- Side bar end -->
	...
	<?php
		...
		<td><?php echo number_format((($rows["sum"]/$rows["contract_amount"])*100),2);?>%</td>
   ...
   <div class="pagination" style="margin-left:5%;margin-top:43%;position:fixed;">
	<?php
		for($page=1;$page<=$number_of_pages;$page++){
			echo '<a href="chiefpage.php?page=' .$page . '" style="background-color:#f44336;display:inline-block;padding:10px 10px;">'.$page. '</a>';
		}
				}
	?>
	</div>
