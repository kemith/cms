<?php
	...
	if(isset($_POST['add'])){
		...
		$id=$row['id'];
		$query = "INSERT INTO masteroll(date,no_of_labours,rate,amount,remarks,user_id,activity_id) VALUES 
		('$date','$no_of_labours','$rate','$amount','$remarks','$id','$activity_id')";
...
<!-- Side bar end -->
	...
	<li><input type="text" name="no_of_labours" class="form-control1" placeholder="No of Labours" required></li>
	<li><input type="text" name="rate" class="form-control1"placeholder="Rate"  ></li>
	<li><input type="text" name="amount" class="form-control1"placeholder="Amount" readonly  ></li></br></br>
	&nbsp;&nbsp;&nbsp;
	...
	include('connection.php');
	$a_id=$_GET['id'];
	$query="SELECT SUM(amount) as sum from masteroll,activity where masteroll.activity_id=activity.id and activity_id='$a_id'";
...
<!--table display-->
	...
	<table align="center" border="2" cellspacing="0" cellpadding="0" style="width:110%;line-height:30px;margin-left:-70px;margin-top:5%;border-collapse:collapse;position:absolute;">
		...
		<!--<td><a href="edit_masteroll.php?editid=<?php echo htmlentities ($row['id']);?>"><img src="images/edit.png" width="25px" style="padding-left:8px" title="Edit"></a>
		<td><a href="delete_masteroll.php?deleteid=<?php echo htmlentities ($row['id']);?>"><img src="images/delete.png" width="30px" style="padding-left:10px" title="Delete"></a></td>
		--></tr>	
		<?php }?>
		<tr>
		<td><b>Total</b></td>
